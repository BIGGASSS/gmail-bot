...
Use `/relog` while Gmail is connected. Its private OAuth link expires after 15 minutes. Choose the **same Gmail account** and grant consent so Google returns a new, nonempty refresh token. Existing authorization and polling remain active until the replacement succeeds; no tokens are revoked by `/relog`. Cancelled, failed, expired, superseded, or wrong-account attempts leave the connection unchanged.
